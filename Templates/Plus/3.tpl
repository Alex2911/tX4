...
if (mysql_num_rows($MyGold)) {
	if($session->gold == 0) {
		...
		echo "<p>You currently have <b> $session->gold </b>  Gold</p> You currently have <b>$silver</b> Silver";
}
}
 ?>
...
if (mysql_num_rows($MyGold)) {
	if($golds['gold'] > 2) {
	echo '<font size="1" color="gray">Go to market';
} else {
	echo '<a href="plus.php?s=1"><span class="none">too little gold';
	}
}
 ?>
        ...
        <td class="desc">Finish training troops in <b>Current</b> Village</td> 
        <td class="dur">now</td> 
        <td class="cost"><img src="img/x.gif" class="gold" alt="Gold" title="Gold" />25</td> 
        <td class="act"><span class="none"> 
<?php 
if (mysql_num_rows($MyGold)) { 
    if($golds['gold'] > 25) { 
    echo ' 
                <a href="tfinish.php"><span>Go'; 
} else { 
    echo '<a href="plus.php?s=1"><span class="none">too little gold'; 
    } 
} 
	...
	<!---
		...
		<thead>
			<tr>
				...
				<th colspan="5"><strike>Travian Gold club</strike></th>
			</tr>
			<tr>
				<td></td>
				<td>Description</td>
