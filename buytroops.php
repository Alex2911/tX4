...
while($row = mysql_fetch_array($sql)){
$gold = $row["gold"];};
if ($gold < 15 ) {die('Sorry you dont have enough gold');}
else {
mysql_query("UPDATE ".TB_PREFIX."users SET `gold` = `gold`- 15 WHERE id ='".$session->uid."'")or die(mysql_error());    
mysql_query("UPDATE ".TB_PREFIX."training SET `commence` = 0 WHERE vref ='".$village->wid."'")or die(mysql_error());
echo "<h2>All Queued Troops In This Village Completed at the Cost of 15 Gold.</h2>";}}?>
<br>
</div>
<div id="side_info">
<?php
